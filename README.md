...
2. To register a file with the bot, **forward it to the bot directly**
   ...
   **In this new updated code, the files are auto indexed by the bot, to allow bulk or large file uploads.**
...
3. **Create `config.py`** - copy `config_example.py` and fill in your
   ...
   # Add AUTO_INDEX_CHANNEL_ID = "-YOUR_CHANNEL_ID" if you want to enable auto-indexing.
   ```
...
5. **Add yourself as the first admin** - put your own Telegram ID in
   ...
   **To Check your Telegram Id - search for @userinfobot and send a message. You'll be provided with numeric ID (e.g.1234567890)**
...
6. **Add your first file** - as an admin, forward any document, video,
   ...
   **As informed above, you can also enable auto-indexing from a Channel by making the bot as admin.**
